...
void affichage_arbre(noeud *racine, int profondeur) {
    if (racine == NULL) {
        printf("/\n");
        return;
    }
    ...
    affichage_arbre(racine->gauche, profondeur+1);

    for (i = 0; i < profondeur+1; i++) {
        printf("\t");
    }
    affichage_arbre(racine->droit, profondeur+1);
